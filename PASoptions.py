...
class OptionsDlg(QDialog, ui_Options.Ui_Options):
	...
	def onChangeManualFile(self):
		filename=QFileDialog.getOpenFileName(self,"Select User Manual File",self.manualFileLE.text(),"(*.html *.htm)")
	...
	def getOptions(self):
		...
		if self.BI_savehistCB.isChecked():options.BI_savehist=unicode(self.BI_savehistLE.text())
		else: options.BI_savehist=''
		#PATHS
