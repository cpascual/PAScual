...
class CHN(object):
	def __init__(self,CHNfile):
		...
		(self.ftype,self.mca_num,self.segment,self.secs,self.realtime,self.livetime,self.acqtime,self.channoffset,self.nchann)=self.hdr
	@staticmethod
	def readCHN(CHNfile):
		...
		datfmt="%ii"%(hdr[8]-hdr[7]) # hdr[8] is the number of channels and hdr[7] is the channel offset
	...
	def toASCII(self, ASCIIfile,ncol=1, hdr="", onError='w', writechannel=False):
		...
		if ncol>1:
			...
			if rm>0:
				nrow+=1
				if onError=='w': print>>sys.stderr, 'Warning: padded with %i zeros'%(ncol-rm)
				elif onError=='n': pass
				else: raise ValueError('Data size does not fit into %i columns'%ncol)
			dat=self.data.copy()
		...
		if writechannel:
			...
			else: raise NonImplementedError("channel numbers not yet supported for multicolumns")
		else: savetxt(ASCIIfile, dat, fmt='%9i')
		ASCIIfile.close()
	def toLT(self, ASCIIfile, description="LT_description", nsperchannel="LT_nsperchannel", key="LT_key", fwhm="LT_fwhm", ncol=1, onError='w'):
