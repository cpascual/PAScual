...
FWHM2SIGMA=1./(2*S.sqrt(2*S.log(2)))  #FWHM = SIGMA*(2*sqrt(2*log(2))) = SIGMA*2.3548200450309493
...
class newcolor(object):
	def __init__(self,i0=0):
		self.i=i0
		self.colorlist=['black','white','red','blue','green','brown','orange','cyan','purple','violet', 'khaki']
		self.i0=i0
	def next(self):
...
#			if isinstance(ob,str): ob=open(ob,'w')
	...
	def write(self, string):
		...
		if self.emitEnabled: emitter.emit(SIGNAL('teeOutput'),copy.deepcopy(string))
	def flush(self):
...
class fitable:
	...
	def _register_perturbable_(self,pert):
		if isinstance(pert,list):
			for ob in pert:
				if ob.free:self.perturbablelist.append(ob)
				else: self.fixedpertlist.append(ob)
		else:
			if pert.free:self.perturbablelist.append(pert)
			else: self.fixedpertlist.append(pert)
		return
	...
	def perturbate(self,npert=1,fitmember=[],direct=False):
		...
		if fitmember:
			...
			for ob in shuffled: changedobjectlist+=ob.perturbate(npert=npert,direct=direct)
		else:
			for i in range(npert):
				changedobjectlist+=random.choice(self.perturbablelist).perturbate(direct=direct)
...
class fitpar_original(fitable):
	def __init__(self,val=0., minval=0., maxval=None, free=False, mstep=0., sstep=0.,name=None):
...
#		super(fitpar_original,self).__init__(free=free,name=name) # This line is a more elegant way of doing: fitable.__init__(self,free=free,name=name)
		...
		self.sstep=sstep
		self.old=val
		if self.minval>self.maxval and self.maxval is not None: raise ValueError('minval cannot be larger than maxval')
...
class fitpar(fitpar_original):
	def __init__(self,val=0., minval=0., maxval=None, free=False, name=""):
		fitpar_original.__init__(self,val=val,minval=minval, maxval=maxval, free=free,name=name)
#		super(fitpar,self).__init__(val=val,minval=minval, maxval=maxval, free=free,name=name)		
	...
	def clearstats(self):
		...
		self.best=None
	def showreport(self):
		print "%s: %.3e (%.3e) [%s - %s]\n\tFree=%s NNRLA: %.3e (%.3e)\n\t%s" %(self.name, self.val, S.sqrt(self.var), self.minval, self.maxval, self.free, self.NNRLA_delta, self.NNRLA_sigma,self)
...
class discretepals(fitable):
	...
	def __init__(self, name=None, expdata=None, roi=None, taulist=None, itylist=None, bg=None, fwhm=None, c0=None, psperchannel=None, area=None, fake=False):
#		self.checklist={'name'=(3>4),
		...
		self.initifready()
	def isready(self):
		self.ready= self.ready or not( ((self.exp is None) and not self.isfake) or (self.isfake and (self.roi is None))  
										or (self.taulist is None) or (self.itylist is None)  or (self.bg is None)  
										or (self.fwhm is None) or (self.c0 is None) or (self.psperchannel is None)
										or (self.isfake and (self.area is None))  )	
		return self.ready
	def initifready(self,force=False):
		if self.realinit and not force: return self.realinit
		if self.isready(): self._real_init(name=self.name, expdata=self.exp, roi=self.roi, taulist=self.taulist, itylist=self.itylist, bg=self.bg, fwhm=self.fwhm, c0=self.c0, psperchannel=self.psperchannel, area=self.area)
		return self.realinit	
	def _real_init(self, name=None, expdata=None, roi=None, taulist=None, itylist=None, bg=None, fwhm=None, c0=None, psperchannel=1, area=1.):
		''' 
...
#		super(discretepals,self).__init__(name=name)
		...
		if expdata is not None:self.exp=S.array(expdata,dtype='d')  #for fitting, self.exp[self.roi] is to be used
		else: self.exp=S.zeros(len(roi),dtype='d')  
		if roi is not None:self.roi=S.array(roi,dtype='i')
		else:self.roi=S.arange(self.exp.size,dtype='i')
		if len(taulist)!=len(itylist): raise TypeError,'taulist and itylist must be of same size'
		for ob in taulist+itylist: 
			if not isinstance(ob,fitpar): raise TypeError("Fitpar wanted")
		self.taulist=taulist
		self.itylist=itylist		
		if isinstance(bg,fitpar): self.bg=bg
		else: raise TypeError("Fitpar wanted")
		if isinstance(fwhm,fitpar): self.fwhm=fwhm
		else: raise TypeError("Fitpar wanted")
		if isinstance(c0,fitpar): self.c0=c0
		else: raise TypeError("Fitpar wanted")
		self.psperchannel=psperchannel
		
		#Register all the perturbables
		self._register_perturbable_(self.taulist + self.itylist + [self.bg, self.fwhm, self.c0])
		...
		for i in xrange(self.ncomp):
			self.tau[i],self.ity[i]=self.taulist[i].val,self.itylist[i].val #filling the tau and ity vectors
			if self.itylist[i].free:self.freeityindexes.append(i)
		self.freeityindexes=S.array(self.freeityindexes,dtype='i') #this contains the indexes of those itys which vary (need normalising)
		self.fixeditysum=self.ity.sum()-self.ity[self.freeityindexes].sum() #the sum of the fixed itys (which won't vary unless the 'free' status of an ity varies)
		self.channeltimes=self.calculate_channeltimes(self.roi,self.c0.val,self.psperchannel) #channel times
		self.M=self.calculate_M(self.channeltimes,self.tau,self.fwhm.val,M=None) #calculating the M matrix
		self.M_dot_a=S.dot(self.M,self.normalizeity(self.ity,self.freeityindexes,self.fixeditysum))
		if self.exp.sum()==0: self.exp[self.roi]=self.fake()		#If no expdata was given, create a fake spectrum and use it as exp data
		self.deltaexp=S.where(self.exp<4,2.,S.sqrt(self.exp))  #A quite good approximation to Poisson statistics (error=2 for 0-3 counts and Gaussian approx for the rest)
		self.deltaexp2=self.deltaexp**2
		self.NNRLA_S0=(1./self.deltaexp2[self.roi]).sum()
		self.exparea=float(self.exp[self.roi].sum())  #Area of exp spectrum in roi, including background
		self.sim=self.calculate_sim(self.bg.val,self.exparea,self.channeltimes,self.M_dot_a)
...
#		print 'DEBUG: !!!!!!!!!!!!!!!!', self.ity,'\n:::::::',self.ity_mean
		self.ity_mean,self.ity_var=updatestats(self.ity_mean,self.ity_var,self.ity_n,itynorm)
		self.ity_n+=1
	...
	def recalculate_chi2(self, full_output=False, forcecalc=False ):
		...
		if not somechange: 
			if full_output: return self.chi2, (self.sim-self.exp[self.roi])/self.deltaexp[self.roi]
			else: return self.chi2
		...
		if self.c0.changed():
			recalc_M=range(self.ncomp)
			self.channeltimes=self.calculate_channeltimes(self.roi,self.c0.val,self.psperchannel)
#		print ";;;;;;;;;;;;;;;;;;;;;;;;;",self.ncomp
		for i in xrange(self.ncomp):
...
#				print "::::::::::::::::::",i, self.itylist[i].name,self.itylist[i].val,self.ity
		...
		if recalc_M:
			self.M=self.calculate_M(t=self.channeltimes,gridtau=self.tau,fwhm=self.fwhm.val,M=self.M,indexarray=recalc_M)
			recalc_M_dot_a=True
		...
		if full_output: return self.chi2, residuals
		else: return self.chi2
	...
	def calculate_channeltimes(self,roi,c0,psperchannel):
		...
		return (roi-c0)*psperchannel #this returns the times in the boundaries between channels
#		return (roi+(0.5-c0))*psperchannel	#this would return the times in the middle of the channels
	...
	def calculate_convoluted_decay(self,t,tau=1.,intsty=1.,sigma=1.):
		...
		return K*erfc((sigma/(S.sqrt(2.)*tau))-(t/(S.sqrt(2.)*sigma)))*S.exp(-t/tau)
		
	def calculate_convoluted_decay_integCh(self,t,tau=1.,intsty=1.,sigma=1.):
		...
		t_extd=S.concatenate((t,(2*t[-1]-t[-2],)))  #the trick is that t_n+Deltat=2*t_n-t_(n-1) 
		s=S.sqrt(2.)*sigma
		#Calculate in one go all the values for Y(tau,tk,s)+phi(tk,s)  of eq. 5 of Kirkegaard
		YplusPhi=S.exp(s*s*0.25/(tau*tau))*erfc((s/(2.*tau))-(t_extd/s))*S.exp(-t_extd/tau) + erfc(t_extd/s)
...
#		return= intsty*(calculate_convoluted_decay(t,tau=tau,sigma=sigma,intsty=alpha) + calculate_convoluted_decay(-t,tau=tauL,sigma=sigma,intsty=alphaL) + calculate_convoluted_decay(t,tau=tauR,sigma=sigma,intsty=alphaR) )
	...
	def calculate_M(self,t,gridtau,fwhm,M=None,indexarray=None):
		...
		if M is None:
			...
			if indexarray is not None: raise ValueError('M must be supplied if indexarray is supplied')
		if indexarray is None: 
			indexarray=S.arange(gridtau.size,dtype='i')
		for i in indexarray:
			M[:,i]=self.calculate_convoluted_decay_integCh(t,tau=gridtau[i],intsty=1.,sigma=sigma)
		return M
...
#		itynorm=100.*self.normalizeityfactor(ity=S.array([ob.mean for ob in self.itylist]))
		...
		retval="%20s\t%14e\t%14e\t%9s\t"%(self.name, self.chi2/self.dof, self.autocorr, string.join([ob.name for ob in self.updatelist],','))
		retval+="%6i\t%6i\t%6i\t%14e\t"%(self.roi[0], self.roi[-1], self.roi.size, self.exparea)
		retval+="%9g\t%9g\t%9g\t%9g\t%9g\t%9g\t"%(self.fwhm.mean, S.sqrt(self.fwhm.var), self.c0.mean, S.sqrt(self.c0.var), self.bg.mean, S.sqrt(self.bg.var))
...
#		for ity,i in zip(self.itylist,range(len(self.itylist))): retval+="%9g\t%9g\t"%(ity.mean*itynorm[i], S.sqrt(ity.var)*itynorm[i])
		for i in xrange(self.ncomp): retval+="%9g\t%9g\t"%(itymean[i], ityerr[i])
		retval+=(2*npad)*("%9g\t"%0)
		for tau in self.taulist: retval+="%9g\t%9g\t"%(tau.mean, S.sqrt(tau.var))
		retval+=(2*npad)*("%9g\t"%0)
		retval+="\n"
...
#		atocorr=self.calculate_residuals_local_correlation()
	...
	def calculate_residuals_local_correlation(self, residuals=None, locallength=None):
		...
		if residuals is None: residuals=(self.sim-self.exp[self.roi])/self.deltaexp[self.roi]
		if locallength is None: locallength=max(5,int(5*self.fwhm.val/self.psperchannel))
		nboxes=residuals.size-locallength+1
		if nboxes<1: raise ValueError("The locallength must be smaller than the size of the residuals")
		result=0
		for i in xrange(nboxes):
			localcorr=S.corrcoef(self.channeltimes[i:i+locallength], residuals[i:i+locallength])[0][1]
	...
	def calculate_NNRLA_SZ(self,fp,direct=False):
		...
		if direct: Z=(((self.exp[self.roi]-self.sim)/self.deltaexp2[self.roi])*fprime).sum()
		else:Z=0
		return S0,S1,S2,Z
	...
	def importvalues(self,other,onlyfree=False, flexicomp=False):
		...
		if self.ncomp==other.ncomp: t1,i1,t2,i2=self.taulist, self.itylist, other.taulist, other.itylist
			...
			for ob in self.ity[other.ncomp:]: success*=ob.importval(0.,onlyfree=onlyfree) #make the other intensities 0
		elif flexicomp and self.ncomp<other.ncomp:  #the extra components are merged into the LAST one
			tomergetau=S.array([fp.v for fp in other.taulist[self.ncomp-1:] ]) #contains lifetimes to merge
			tomergeity=S.array([fp.v for fp in other.itylist[self.ncomp-1:] ]) #contains intensities to merge
			...
			t2,i2=other.taulist[:self.ncomp-1]+[mergedtau], other.itylist[:self.ncomp-1]+[mergedity]
	...
	def saveAs_ASCII(self, f,hdr=None, columns=1, datafmt='%i'):
		...
		if columns>1: raise NonImplementedError() #TODO
		S.savetxt(f,self.exp,fmt=datafmt)
		f.close()
	def saveAs_LT(self, f):
		hdr="%s\n%.4g\n%g\n%.4g\n"%('LT_description',self.psperchannel*1e-3,-1234567,self.fwhm.val*1e-3)
...
#		super(palsset,self).__init__(name=name)	
	...
	def goodItyErrors(self):
		...
		if self.spectralist[0].ncomp<1: return False #the first spectrum has no components (BAD)
		#Check that all spectra in the set are have the same number of free components
		for dp in self.spectralist[1:]: 
			if dp.freeityindexes.size!=self.spectralist[0].freeityindexes.size: return False
		#then check that the itys are either completely independent or completely dependent (i.e. all common or none common)
		common=0
		itysA=[self.spectralist[0].itylist[i] for i in self.spectralist[0].freeityindexes]
		itysA.sort()
		...
		for dp in self.spectralist[1:]: 
			...
			common+=(itysA==itysB)
		if common==len(self.spectralist[1:]): return True #then all components have the same free parameters (GOOD)
		elif len(unique(allitys))==len(allitys): return True #there are not common itys at all (GOOD)
	...
	def register_spectrum(self,spectrum):
		...
		self.roimin,self.roimax=min(self.roimin,spectrum.roi[0]), max(self.roimax,spectrum.roi[-1])
#	def upgrade_spectrum(self, 
		
	def confirm(self,savehist=False):
		'''confirm the various elements that may change and then proceed to the standard _confirm_()'''
		for ob in self.fitparlist: ob.updatestats(savehist=savehist) #update stats for all fitpars (including the ity pars, which is a waste)
		for ob in self.spectralist: ob.updatestats_ity()#update stats on itys (this must be done for each spectrum separately)
		self.chi2_old=self.chi2
		self.chi2_mean,self.chi2_var=updatestats(self.chi2_mean,self.chi2_var,self.n,self.chi2)
...
#				SZ[0]=0
			...
			if direct:
				...
				if fp.NNRLA_delta>maxstep: fp.NNRLA_delta=maxstep
				elif fp.NNRLA_delta<-maxstep: fp.NNRLA_delta=-maxstep
				#calculate the diff
			#calculate the sigma
			fp.NNRLA_sigma=S.sqrt(SZ[0]/(SZ[0]*SZ[2]-SZ[1]*SZ[1]))
			#apply limits to sigma
			if fp.NNRLA_sigma>maxstep: fp.NNRLA_sigma=maxstep
			elif fp.NNRLA_sigma<-maxstep: fp.NNRLA_sigma=-maxstep
			#calculate the diff
			fp.NNRLA_diff=abs(((deltaold-fp.NNRLA_delta+sigmaold-fp.NNRLA_sigma)/(abs(deltaold)+sigmaold)))
			diff+=fp.NNRLA_diff
#			print "DEBUG:%s %g %g (%g) %g (%g)"%( fp.name,fp.val, fp.NNRLA_delta, SZ[3]/SZ[2],fp.NNRLA_sigma, S.sqrt(SZ[0]/(SZ[0]*SZ[2]-SZ[1]*SZ[1])))
		try: diff/=len(self.fitparlist)
		except :pass
#		print ':::::::::::::::::::::::::',diff
		return diff
	def clearstats(self):
	...
	def MCMC_generate(self, LM=None, T=2., ireport=-1, savehist=False, NNRLA='auto', direct=False, factor=1,iemit=-1):
		...
		nemit=iemit
		self.NNRLAcount=0
		if NNRLA=='auto': NNRLA=autoNNRLA=True
		else: 
			autoNNRLA=False
		...
		while acc<LM:
			...
			try :perttarget=perttargetlist.pop()
			except IndexError:
				perttargetlist=1*self.fitparlist
			...
			if acceptflag:
				...
				if icalcNNRLA==0: 
					...
					if autoNNRLA: #If the auto adjustment of NNRLA is required, do it . TODO: the thressholds are quite arbitrary (fix?)
						if   NNRLA_diff>.2: NNRLA=max(1,NNRLA/2) 
						elif NNRLA_diff<.05: NNRLA=min(LM/5,nreport,NNRLA*2)
...
#					self.graph_report()
				if(iemit==0):
					 emitter.emit(SIGNAL("commandPBarValue(int)"),acc) #signal for updating the progress bar
					 iemit=nemit
			else: self.undo()
			
	def showreport_1row(self, file=None, min_ncomp=None, header=False):
		if min_ncomp is None: min_ncomp=max([ob.ncomp for ob in self.spectralist])
		if header: 
			file.write(time.asctime())
			file.write("\n%20s\t%14s\t%14s\t%9s\t"%("name","chi2","autocorr","Set(s)"))
			file.write("%6s\t%6s\t%6s\t%14s\t"%("ROImin","ROImax","ROIch","Integral"))
			file.write("%9s\t%9s\t%9s\t%9s\t%9s\t%9s\t"%("FWHM","dev","c0","dev","bg","dev"))
			for i in xrange(1,min_ncomp+1): file.write("%9s\t%9s\t"%("ity%i"%i,"dev"))
			for i in xrange(1,min_ncomp+1): file.write("%9s\t%9s\t"%("tau%i"%i,"dev"))
			file.write("\n")
		for ob in self.spectralist: ob.showreport_1row(file=file, min_ncomp=min_ncomp)
	...
	def showreport(self,T=None,acc=None, verbosity=0, NNRLA=-1):
		...
		if T: print "T= %.2e "%(T)
		print "X2= %.3e(mean)  %.3e(min)  %.3e(deviation)  %.3e(curr)"%(self.chi2_mean,self.chi2_best,S.sqrt(self.chi2_var),self.chi2)
		if acc is not None: 
			print "accepted: %i  (%.1f%% , %.1f%%increased)"%(acc, self.accratio*100, self.inc_dec_ratio*100)
	...
	def graph_report(self,filename=None,show=True):
		...
		pylab.plot([self.roimin,self.roimax],[1,1],color='black') #draw a base line for reference
		pylab.plot([self.roimin,self.roimax],[-1,-1],color='black') #draw a base line for reference		
		print 'Plot legend:'
		for sp in self.spectralist:
			...
			pylab.subplot(211)
			if sp.exp[sp.roi].sum()>0: pylab.plot(sp.roi,sp.exp[sp.roi],linewidth=0,markerfacecolor=col , marker='o', markersize=3.)
			pylab.plot(sp.roi,sp.sim,color=col,linestyle='-')
			pylab.subplot(212)
			pylab.plot(sp.roi,residuals,markerfacecolor=col, linewidth=0,marker='o', markersize=3. )				
		if filename is not None: 
			pylab.savefig(filename)
			print "\nGraphic output saved to '%s'\n"%filename
		if show: pylab.show()
		
	def simann(self,LM=None,startT=2.,stopT=1e-3,stepT=.9,maxiter=S.inf,minaccratio=0.1, meltratio=0.97,tolerance=None, direct=True, LMmult=20, chi2min=1.):
		...
		while self.accratio<meltratio:
			...
			if abort.abortRequested():return #check if we should abort
		#The SA loop
		emitter.emit(SIGNAL("initCommandPBar(int,int)"),int(-S.log(T)),int(-S.log(stopT))) #signal for setting up the progress bar
		while not endflag:
			self.MCMC_generate(LM=LM, T=T, ireport=LM, NNRLA='auto', direct=(direct and iter>4))
			if abort.abortRequested():return #check if we should abort
			T*=stepT
			iter+=1
			emitter.emit(SIGNAL("commandPBarValue(int)"),int(-S.log(T))) #signal for updating the progress bar
	...
	def BI(self,LM, stabilisation=0., ireport=None, factor=None,iemit=-1, savehist=False):
		...
		#do stabilisation
		if stabilisation>0:
			emitter.emit(SIGNAL("initCommandPBar(int,int)"),0,int((stabilisation+LM)*factor)) #signal for setting up the progress bar
			self.MCMC_generate(LM=stabilisation, T=2., ireport=min(ireport,stabilisation), savehist=False, NNRLA='auto', direct=False, factor=factor,iemit=iemit)
		if abort.abortRequested():return #check if we should abort
		#and do the proper BI
		emitter.emit(SIGNAL("initCommandPBar(int,int)"),int(-stabilisation*factor),int(LM*factor)) #signal for setting up the progress bar with an illusion of continuity
		self.MCMC_generate(LM=LM, T=2., ireport=ireport, savehist=bool(savehist), NNRLA='auto', direct=False, factor=factor,iemit=iemit)
		if abort.abortRequested():return #check if we should abort
	...
	def localmin(self, maxunbound=0, ireport=False, forcelimits=True):
		...
		'''
		emitter.emit(SIGNAL("initCommandPBar(int,int)"),0,5) #signal for setting up the progress bar
		...
		for ob in itys: #clean the list of non-free intensities
			if ob.free: temp.append(ob)
			else: nonfreeitys+=1
		itys=temp
		nitys=len(itys)
		for i in range(nitys): itys[i]=objectindex(itys[i],myargs) #itys now contains the indexes for the parameters that are intensities in myargs
		itys=S.array(itys)
		...
		emitter.emit(SIGNAL("commandPBarValue(int)"),1) #signal for updating the progress bar
#		self.showreport(verbosity=1)
		if abort.abortRequested():return #check if we should abort
		myx, cov_x, infodict, mesg, ier= optimize.leastsq(self.interfacefunction_leastsq, myx, tuple(myargs), Dfun=None, full_output=1, col_deriv=0, maxfev=0, epsfcn=0.0, factor=100, diag=None)
...
#		if ireport: self.showreport(verbosity=1)#DEBUG
		...
		if not withinlimits and forcelimits: 
			if ireport: 
				...
				for fp in myargs:
					if fp.maxval is None: maxval=S.inf
					else: maxval=fp.maxval
					if not ( fp.minval<fp.val<maxval): fp.showreport()
				print "\nTrying a bounded minimisation (may be slow)\nProgress:",
			#reset the state
			self.undolist+=myargs
			self.undo()
			myargs,myx,minmax=self.get_fitpars(val=True,minmax=True,onlyfree=True) #redo the mapping
			#Do a  L-BFGS-B minimisation (slow but bounded)
			if abort.abortRequested():return #check if we should abort
			myx,chi2,infodict=optimize.fmin_l_bfgs_b(self.interfacefunction,myx,args=myargs, approx_grad=1, bounds=minmax, m=10, iprint=-1)
			emitter.emit(SIGNAL("commandPBarValue(int)"),2) #signal for updating the progress bar
			if ireport:print '#',
...
#				print mesg
				if ireport:print '>',
			emitter.emit(SIGNAL("commandPBarValue(int)"),3) #signal for updating the progress bar
...
#			myx=S.where(myx>minmaxarray[:,1], minmaxarray[:,1], myx)
			if abort.abortRequested():return #check if we should abort
			myx,chi2,infodict=optimize.fmin_l_bfgs_b(self.interfacefunction,myx,args=myargs, approx_grad=1, bounds=minmax, m=10, iprint=-1)
			emitter.emit(SIGNAL("commandPBarValue(int)"),4) #signal for updating the progress bar
			if ireport:print '#\n'
		#Confirm and show the results
		emitter.emit(SIGNAL("commandPBarValue(int)"),5) #signal for updating the progress bar
		self.calculate_chi2(recalc=True)
		...
		for dp in self.spectralist:
			if dp.freeityindexes.size>0:
				...
				for i,j in  zip(dp.freeityindexes,xrange(aindexes.size)): 
					aindexes[j]=objectindex(dp.itylist[i],myargs) #aindexes contains the indexes (in myargs) of the free intensities for this spectrum
				a=myx[aindexes]
				asum=a.sum()		
				...
				for i in aindexes:
					Jac[i,aindexes]=myx[i] #the row corresponding to the intensity a_i is all equal to a_i...
					Jac[i,i]-=asum         # ...except for the diagonal, where it is (a_i-sum{a})
					Jac[i,aindexes]*=(1-dp.fixeditysum)/(asum**2) #Jac_ij= (norm_free/sum_k{a_k})(a_i-dirac_ij*sum_k{a_k})    ... for i,j,k running over the free intensities
				#The new covariance matrix is given by cov_new=J.cov.J^T  
				cov_new=S.dot(Jac,S.dot(cov_x,Jac.T))	
				dp.ity_var[dp.freeityindexes]=S.diag(cov_new)[aindexes]  #fill the ity_var array for each spectrum
	...
	def compatible(self, other, detail=False):
		...
		temp=True
		for ob1,ob2 in zip(self.spectralist,other.spectralist): temp*=(ob1.ncomp==ob2.ncomp)
		res["ncomp"]=temp
		temp=True
...
#			if xmap[i].val!=x[i]: xmap[i].val=x[i]
	...
	def get_fitpars(self,val=True,minmax=True,onlyfree=True):
		...
		if onlyfree: fpmap=self.unfold_perturbablelist()
		else:
			fpmap=[]
			for ob in self.spectralist: fpmap+= ob.itylist + ob.taulist + [ob.bg, ob.fwhm, ob.c0]
			fpmap=unique(fpmap)
		if minmax: return fpmap,S.array([ob.val for ob in fpmap]),[(ob.minval,ob.maxval) for ob in fpmap]
		elif val: return fpmap,S.array([ob.val for ob in fpmap])
		else: return fpmap
...
#		cmax=S.array(pylab.load(expfilename,skiprows=headerlines),dtype='d').argmax(0) 
	...
	else: cmax=S.array(expdata,dtype='d').argmax(0)
	roi=S.arange(cmax-left_of_max,stopdat+1)
	return roi

def to_list(N, parameter):
	if isinstance(parameter,list): 
		if len(parameter)!=N: raise ValueError("The number of elements of the list (%i) does not match N (%i)"%(len(parameter),N))
	else: parameter=N*[parameter]
	return parameter
...
def assignfitpar(v, namedpars=None):
	...
	if isinstance(v,str): 
		if v[-1]=='*': return  namedpars[v[:-1]] #returns the named parameter
		else: return copy.deepcopy(namedpars[v]) #returns a COPY of named parameter
	elif S.size(v)==1: return fitpar(val=v, name='!', free=False)
	else: return fitpar(val=v[0], name='@', minval=v[1],maxval=v[2],free=True)
...
def findconnectedspectra(spectrum):
	...
	n_old=0
	n=1
	while (n_old<n): #This is done while the list is growing (i.e., while new connected spectra ar found)
		for el in L1:  #go through all elements in the connected list
			for fp in el.perturbablelist: #and go through all their fitpars
				if len(fp.updatelist)>1: #note, if len<1 then the only element must be "el" so we don't waste time adding it
	...
	for i in xrange(len(connections)):
		if not exclude.count(i): #if i is not amongst the already excluded
			for j in xrange(i+1,len(connections)):
				if len(connections[i])==len(unique(connections[i]+connections[j])): #this is true only the elements of connections[j] are the same as those of  connections[i]
					exclude.append(j)
...
def mainprogram(warningslog=[]):
	...
	try: temp=userinput.namedparameters
	except AttributeError: userinput.namedparameters={}
	namedpars={}
	...
	for k in userinput.namedparameters.keys():
		if isinstance(userinput.namedparameters[k],str): warningslog+=printwarning("Defining a named parameter using another named parameter may lead to problems. You are warned.",wait=True)
		namedpars[k]=userinput.namedparameters[k]=assignfitpar(userinput.namedparameters[k],namedpars)

	###find out the number of spectra to be fitted
	if not isinstance(userinput.expfilenames,list): userinput.expfilenames=[userinput.expfilenames]
	try: 
		if not isinstance(userinput.skipfilenames,list): userinput.skipfilenames=[userinput.skipfilenames]
	except AttributeError: userinput.skipfilenames=[]
	for fn in userinput.skipfilenames: #eliminate the ones that the user wants to skip:
		try: userinput.expfilenames.remove(fn)
	...
	for fname,hdrlns in zip(userinput.expfilenames,userinput.headerlines):
		print '.',
		if fname is None: expdata.append(None) #No exp data is given (we will only simulate)
		else: 
#			expdata.append(S.array(pylab.load(fname,skiprows=hdrlns),dtype='d').flatten()) # load datafile: ascii format
			expdata.append(S.loadtxt(fname,skiprows=hdrlns,dtype='d').flatten())
#	
#	for es in expdata:
#		pylab.cla()	
#		pylab.gca().set_yscale('log')
...
#		pylab.show()
	...
	try: userinput.roi=to_list(nspectra, userinput.roi) #explicit ROI definition
		...
		userinput.roi=[]
		for i in xrange(nspectra): userinput.roi.append(MELTlikeROI(expdata[i], headerlines=userinput.headerlines[i], left_of_max=userinput.left_of_max[i], stopdat=userinput.stopdat[i]))
	try: userinput.bg=to_list(nspectra, userinput.bg) #explicit bg definition
	...
	try: userinput.c0=to_list(nspectra, userinput.c0)#explicit c0 definition
	except AttributeError: userinput.c0=to_list(nspectra, None)#if not explicit, c0 will be calculated later from the spectrum
	try: userinput.ity=to_list(nspectra, userinput.ity) #explicit ity definition
	except AttributeError: userinput.ity=[nc*((1,0,None),) for nc in userinput.ncomp] #if not explicit, init them all the same (and free parameters)
	...
	for i in xrange(nspectra):
		...
		#Assign c0
		if userinput.c0[i] is not None: c0=assignfitpar(userinput.c0[i],namedpars)
		else:
		...
		if userinput.bg[i] is not None:
			...
			temp2=10*max(10,S.sqrt(temp1),expdata[i][userinput.startbg[i]:userinput.stopbg[i]].std())
			bg=assignfitpar((temp1,max(0.,temp1-temp2),temp1+temp2),namedpars)
		#assign tau
...
#		raw_input()
		...
		if userinput.expfilenames[i] is not None: 
			...
			dpname='FAKE%i'%i
			fake=True
		spectra.append(discretepals(name=dpname, expdata=expdata[i], roi=userinput.roi[i], taulist=taulist, itylist=itylist, bg=bg, fwhm=fwhm, c0=c0, psperchannel=userinput.psperchannel[i],fake=fake,area=1))
...
#	raw_input()
	...
	for iset in xrange(npalssets):
		...
		print "*****%s*****"%temp
		print (starwidth*"*")+"\n"
		print '%s is comprised of the following spectra: '%palssetslist[iset].name,
		...
		for text in userinput.fitmode[iset]:
			try:#This try-except block is here to catch CTRL+C presses while executing a given program
				...
				if len(text)==2: args=text[1] #the arguments are not case insensitive *by default*
				else:args=None
				#LOAD command
				if cmd=='LOAD':
					...
					if args: #load from file
						...
						try: saveslot=pickle.load(open(args,'rb'))
						except IOError: saveslot=None
					else: saveslot=saveslot_user #load from saveslot_user slot
					if saveslot: #check if there is something to load 
						temp=copy.deepcopy(palssetslist[iset]) #save a backup before attempting import
						success=palssetslist[iset].importvalues(saveslot, onlyfree=False, flexicomp=False)#import the values
						if not success: # if the import was not satisfactory, undo changes
							warningslog+=printwarning("Cannot LOAD. %s is not compatible with %s"%(palssetslist[iset].name,saveslot.name))
							palssetslist[iset]=copy.deepcopy(temp) 
					else: warningslog+=printwarning("Cannot LOAD. Nothing previously saved")
				...
				#SA command	
					...
					palssetslist[iset].simann(minaccratio=0.1, direct=userinput.SA_direct[iset], stopT=userinput.SA_stopT[iset], maxiter=userinput.SA_maxiter[iset], tolerance=userinput.SA_tol[iset], meltratio=userinput.SA_meltratio[iset])
				elif cmd=='LOCAL':
					if args:
					...
					temp=copy.deepcopy(palssetslist[iset]) #making backup in case LOCAL minimisation fails
					try: palssetslist[iset].localmin(maxunbound=10, ireport=True, forcelimits=forcelimits)
					except ValueError: 
						warningslog+printwarning("LOCAL minimisation of %s failed. Skipping!"%palssetslist[iset].name)
						palssetslist[iset]=temp #recovering original from backup
				#BI command	
				elif cmd=='BI'	:
					print '\n********* Performing Bayesian Inference **************\n'
					palssetslist[iset].BI(LM=userinput.BI_length[iset], stabilisation=userinput.BI_stab[iset], ireport=userinput.BI_report[iset], savehist=userinput.BI_savehist)
				#SAVE command	
				elif cmd=='SAVE':
					print '\n********* Saving results **************\n'
					...
					if args: 
						pickle.dump(saveslot_user,open(args,'wb'),-1) #if a filename is provided, save a copy there
				...
				#REPORT command
					...
					if args:
						if args.upper().strip()=='G': palssetslist[iset].graph_report() 
				#FAKE command.
				elif cmd=='FAKE':
...
#					temp=copy.deepcopy(palssetslist[iset])
					temp=palssetslist[iset] #not a deepcopy because we don't need to backup
...
#							S.savetxt(filename+suffix,sp.exp)
					...
					header=False
					if args:
						if args in logfilesdict: logfile=logfilesdict[args]
						else:
							logfile=open(args,'a') #the file is opened in append mode
							logfilesdict[args]=logfile
							header=True
					else: logfile=None
					palssetslist[iset].showreport_1row(file=logfile, min_ncomp=max([ob.ncomp for ob in spectra]), header=header)
				#ADDCOMPONENT command				
				elif cmd=='ADDCOMPONENT':
					palssetslist[iset]=palssetslist[iset].addcomponent() #TODO
				#UNRECOGNISED command
				else:
					warningslog+=printwarning("Command not recognised (%s). Skipping\n"%cmd)
			...
			#Handle Keyboard interrupts
				...
				warningslog+=printwarning(' %s command interrupted. \nPress CTRL+C again to exit completely (or ENTER to skip this command only)'%cmd, wait=True)
				pass
			
							
def safemain():
	warningslog=[]
	try: mainprogram(warningslog) 
	except:
		printwarning('\nSome problem Occurred during the execution.\nMaybe some previous warning (summarised below) may give a hint.\nCheck also the error messages below.', wait=True)
...
def test_component():
	...
	y1=bbg+dpp.calculate_convoluted_decay_integCh(tt,tau=ttau,intsty=AA,sigma=100.)
