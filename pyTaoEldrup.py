...
def RTE_F(x,T,delta=0.19,tol=1e-10, maxorder=100):
	...
	if order>1:
		while (not convergence and i<order):
			...
			convergence= (( den_i/den_sum<tol and  abs(num_i)/abs(num_sum)<tol ) or term/F<tol) or not S.isfinite(term)
	else:
...
def RectangularTaoEldrup(a,b,c,T,delta=0.19):
	...
	'''
	rate=2.0053-1.9982*RTE_F(a,delta=delta,T=T)*RTE_F(b,delta=delta,T=T)*RTE_F(c,delta=delta,T=T)
	return 1./rate
