...
class ASCIIfileloader(spectrumFileLoader):
	def __init__(self, name='ASCII',filenamefilter='*',hdrlines=None, description=None, askmode='t', Qtparent=None ):
		spectrumFileLoader.__init__(self,name)
		self.filenamefilter= filenamefilter
		if description is None: self.formatDescription='File containing yields separated by any blank space'
		else:self.formatDescription=description
	...
	def askExtraInput(self,mode='t'):
		...
		if self.askmode=='t':
			#asks in text mode
			self.hdrlns=self.int(raw_input('Number of lines to skip in the header?'))
			okflag=True
		if self.askmode=='qt':
			#Asks via a qt dialog. This assumes that Qt is installed. You must provide a parent for the dialog
			from PyQt4.QtGui import QInputDialog
			if self.hdrlns is None: dflt=0
			else: dflt=self.hdrlns
