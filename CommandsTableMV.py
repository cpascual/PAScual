...
class command(object):
	def __init__(self,cmd=u'',args=u''):
		...
		if self.cmd not in knowncommands: raise KeyError('Unknown fitting command (%s)'%cmd)
...
class CommandTableModel(QAbstractTableModel):	
	...
	def loadData(self,commandlist,argslist=None):
		...
		if len(commandlist)!=len(argslist): raise ValueError('CommandTableModel.loadData: commands and args must be the same len')
...
# 		for c in self.commands: print 'DEBUG:',c.cmd
	...
	def data(self, index, role=Qt.DisplayRole):
		...
		if role == Qt.DisplayRole:
			if column == CMD: return QVariant(QString(self.commands[row].cmd))
			elif column == ARGS: return QVariant(QString(self.commands[row].args))
		#Alignment 
...
# 			return QVariant(int(Qt.AlignHCenter|Qt.AlignVCenter))
	...
	def headerData(self, section, orientation, role=Qt.DisplayRole):
		...
		if orientation == Qt.Horizontal:
			if section == CMD: return QVariant("Command")
			elif section == ARGS: return QVariant("Args")
			return QVariant()
		else:
	...
	def setData(self, index, value=None, role=Qt.EditRole):
		if index.isValid() and (0 <= index.row() < self.rowCount()):
			...
			if column==CMD:	
				...
				if value==u"END": self.removeRows( row+1,rows=self.rowCount()-row)
				elif row==(self.rowCount()-1): self.insertRows(position=self.rowCount(),rows=1)
				self.setData(self.index(row,ARGS),value=QVariant(QString())) #clear the args
			elif column==ARGS: self.commands[row].args=value
...
class commandDelegate(QItemDelegate):
	...
	def createEditor(self,parent,option,index):
		if index.column()==CMD:
			...
			return combobox
		else: return QItemDelegate.createEditor(self, parent, option, index)
	def setEditorData(self, editor, index):
		...
		else: QItemDelegate.setEditorData(self, editor, index)
	def setModelData(self, editor, model, index):
		if index.column() ==CMD: model.setData(index, QVariant(editor.currentText()))
		else: QItemDelegate.setModelData(self, editor, model, index)
