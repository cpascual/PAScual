...
# 		self.resultsTable.clear()
		...
		#Adding actions
		self.resultsTable.addActions([self.actionCopy_Results_Selection]) #context menu
	...
	def onCalculate(self):
		...
		#read the input 
		tau=unicode(self.tauTE.toPlainText()).replace(';',' ').split() #semicolons are also valid separators (apart from blank space)
		T=unicode(self.TempTE.toPlainText()).replace(';',' ').split() #semicolons are also valid separators (apart from blank space)
		try:
			tau = S.array([float(elem) for elem in tau],dtype='d')
			T= S.array([float(elem) for elem in T],dtype='d')
		except ValueError:
			QMessageBox.critical(self, "Bad input"," The input for the lifetime/temperature must be a number or sequence of numbers")
			return
		#Check input lengths
		if tau.size==0: 
			QMessageBox.critical(self, "No input"," You must provide at least one lifetime value")
			return
		if self.GeomSphereRB.isChecked(): T=S.zeros(tau.size) #Temp is not used in this model
		if T.size==0:
			QMessageBox.critical(self, "No input"," You must provide at least one temperature value for this geometry")
			return
		if tau.size>1 and T.size==1 and not self.sameTempCB.isChecked(): 
			answer=QMessageBox.warning(self, "Same temperature?","Do you want to use T=%g for all the lifetimes?"%T[0],QMessageBox.Yes|QMessageBox.Cancel)
			if answer==QMessageBox.Yes:
				self.sameTempCB.setChecked(True)
			else: return
		if self.sameTempCB.isChecked() and self.sameTempCB.isEnabled(): 
			if T.size==1: T=T[0]*S.ones(tau.size)
			else: 
				QMessageBox.critical(self, "Bad Input","""Give an unique temperature or uncheck the "same temp" option""")
				return
		
		if T.size!=tau.size:
			QMessageBox.critical(self, "Bad input"," The number of lifetimes (%i) does not match the number of temperatures (%i)"%(tau.size,T.size))
			return
		...
		for i in range(tau.size):
			try:
				if self.GeomSphereRB.isChecked():
					...
					size[i]=a/2.-TE_softwall #this gives the radius of an sphere with equivalent mean free path to the calculated cube
					if self.softwallsCB.isChecked(): size[i]+=TE_softwall
				elif self.GeomCubeRB.isChecked():
					size[i]=RTE_cube(tau[i],T[i])
					if not self.softwallsCB.isChecked(): size[i]-=2*RTE_softwall
		...
		if badcalcsflag: 
			QMessageBox.warning(self, "Possible bad results"," One or more results may not be correct due to bad inversion of the (R)TE formula")	
		...
		if self.psRB.isChecked(): 
			...
			tau*=1000
		else: self.resultsHeader[0]="Lifetime (ns)"
		if self.celsiusRB.isChecked():
			self.resultsHeader[1]="Temperature (C)"
			T-=273
		else: self.resultsHeader[1]="Temperature (K)"
