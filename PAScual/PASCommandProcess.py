    ...
    def launchFit(self, ps):
        ...
        for ob, icmd in zip(ps.commands, list(range(len(ps.commands)))):
            ...
            if cmd == "SA":
                ...
                ps.simann(
                    minaccratio=self.options.SA_minaccratio,
                ...
                try:
                    ps.localmin(
                        maxunbound=self.options.LOCAL_maxUnbound,
                        ireport=True,
            ...
            # BI command
                ...
                ps.BI(
                    LM=self.options.BI_length,
                    stabilisation=self.options.BI_stab,
                    ireport=self.options.BI_report,
                    iemit=1,
                    savehist=savehist,
                )
            # LOAD command
                ...
                if self.saveslot:  # check if there is something to load
                    temp = copy.deepcopy(ps)
                    success = ps.importvalues(
                        self.saveslot, onlyfree=False, flexicomp=False
                    )  # import the values
            ...
            # SAVE command
                ...
                if args:
                    pickle.dump(
                        self.saveslot_user, open(args, "wb"), -1
