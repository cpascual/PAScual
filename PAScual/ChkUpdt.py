...
class updater(QDialog):
    def __init__(self, parent=None, WindowTitle=None, text='', currver='0.0.0',
                 url=''):
    ...
    def check_for_Updates(self, url=None):
        ...
        while (answer == QMessageBox.Retry):
            ...
            answer = QMessageBox.warning(self, "Update Check Failed",
                                         """PAScual could not fetch the latest release information"""
                                         """<p>Typical causes are: <ul>"""
                                         """<li>A problem with the server</li>"""
                                         """<li>Your firewall blocking PAScual</li></ul></p>""",
        ...
        if lastrls is None:
            ...
            if self.compareversions(self.currver, lastrls) < 0:
                updateurl = url[:-3] + "php"
                QMessageBox.information(self, "Update recommended",
                                        """<p>You are currently running v%s.</p>"""
                                        ...
                                        """<p>Visit <a href="%s">%s</a> to download the newest version</p>""" % (
                                        self.currver, lastrls, updateurl,
                                        updateurl))
            else:
                QMessageBox.information(self, "Current version is up-to-date",
                                        """<p>The version that you are running (v%s) seems up-to-date.</p>"""
                                        """<p>If you want to be informed of new releases of PAScual,"""
                                        """consider joining the <a href="https://lists.sourceforge.net/lists/listinfo/pascual-info">PAScual-Info mailing list</a>.</p>""" % (
                                        self.currver))
            self.accept()  # close the updater dialog

    def onClose(self, result=None):
        self.nextupdatechk = int(
            time.time() + 3600 * 24 * self.daysSB.value())  # schedule next check


if __name__ == "__main__":
    app = QApplication(sys.argv)
    form = updater(None, "Updater", "Do you want to check for updates?",
                   "1.0.1", "http://pascual.sf.net/lastrls.txt")
    form.show()
    sys.exit(app.exec_())
