...
Translates oPs lifetime into pore size using the Tao-Eldrup model and also the 
...
def TaoEldrup(R, deltaR=0.1656):
    ...
    G = 1.0 - x + 0.5 * S.sin(2.0 * S.pi * x) / S.pi
    rate = 2.0053 * G
    return 1.0 / rate
...
def RTE_F(x, T, delta=0.19, tol=1e-10, maxorder=100):
    """Calculates the F function of pg 4659 of
    TL Dull et al., J. Phys Chem B 105, 4657 (2001)
    x is the dimension (in nm)
    delta is the free parameter (0.18-0.19 nm)
    T is temperature in Kelvin
    order is the summation order (it should be inf)
    note, in the exponential, a constant value (2181.647 K nm^2)
    is used which is beta/k, where
    beta=(h**2)/16m= 0.188 eV nm^2 and k=8.617343e-5 eV/K

    NOTE: this routine may need to be rewritten, specially for
    performing the sums of F in inverse order, to get rid of the
    finite precision errors in the sumation
    """
    if not S.isfinite(x):
        return 1.0
    F = 1.0 - 2.0 * delta / x
    ...
    if order > 1:
        while not convergence and i < order:
            ...
            convergence = (
                (den_i / den_sum < tol and abs(num_i) / abs(num_sum) < tol)
                or term / F < tol
            ) or not S.isfinite(term)
    else:
...
def RectangularTaoEldrup(a, b, c, T, delta=0.19):
    """The Rectangular Tao Eldrup Formula:
    see TL Dull et al., J. Phys Chem B 105, 4657 (2001)

    Note: the enhancements of this model over the original Tao-Eldrup are:
        1. Cuboid pores are considered (hence allowing for channel or
          lamellar-like geometries to be calculated)
    ...
    delta is an empirical parameter for the T-E model, normally=0.19nm
    (calculated to fit the TE model for R<1nm)
    ...
    Note: The mean free path in a cube with side a=2*(R+deltaR) is the same as in a
        sphere of radius R+deltaR
    """
    rate = 2.0053 - 1.9982 * RTE_F(a, delta=delta, T=T) * RTE_F(
        b, delta=delta, T=T
    ) * RTE_F(c, delta=delta, T=T)
    return 1.0 / rate


def RTE_cube(tau, T, guess=3.0, delta=0.19):
    ...
    Note, it has been tested for values of tau in the ??? ns range and values of T
    in the range of ???
    """
...
def RTE_channel(tau, T, guess=3.0, delta=0.19):
...
def RTE_sheet(tau, T, guess=3.0, delta=0.19):
