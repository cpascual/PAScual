...
'''
This is a modified version of the makepyqt.pyw program by Qtrac Ltd.
Modifications by CarlosPascual-Izarra: changed behaviour for qrc files to match the assumed prefferences from pyuic4

So instead of generating resource files called qrc_something.py the name will be something_rc.py
            ...
            if target.endswith(".py") or target.endswith(".pyc") or \
                ...
                if name.startswith("ui_") and not name[-1] in "oc":
                    ...
                    if target[-1] in "oc":
                        ...
                        source = os.path.join(path, name[0:-6] + ".qrc")
                elif target[-1] in "oc":
                    source = target[:-1]
